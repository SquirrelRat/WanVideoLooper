...
# ====================================================================================================
def _align_to_multiple(x, multiple=16):
    """Aligns a number to the nearest larger multiple."""
    return max(multiple, (int(x) // multiple) * multiple)

def _get_model_device_info(model) -> Tuple[torch.device, torch.dtype]:
    """Gets the device and dtype from a model."""
    try:
...
def _move_tensor(tensor: torch.Tensor, device, dtype):
    """Moves a tensor to a device/dtype if it's not already there."""
...
def _apply_sigma_shift(model, sigma_shift):
    """Applies sigma shift to the model's sampling parameters."""
    ...
    model_sampler.set_parameters(shift=sigma_shift, multiplier=1000)
    model.add_object_patch("model_sampling", model_sampler)
    return model

def _find_model_switch_index(model, scheduler_name, total_steps, boundary_point):
    """Calculates the step index to switch models based on the boundary."""
    try:
        model_sampler = model.get_model_object("model_sampling")
        ...
        for i, t in enumerate(timesteps):
            if t < boundary_point:
                switch_index = max(0, i - 1)
                return switch_index
        return total_steps
    except Exception as e:
        _log(f"Warning: Error calculating model switch point, defaulting to 50/50 split. Details: {e}")
...
def _run_sampler_step(model, seed, steps, cfg, sampler_name, scheduler, positive, negative, latent, denoise, disable_noise=False, start_step=None, last_step=None,  force_full_denoise=False):
    """Thin wrapper for the comfy KSampler node call."""
    ...
    output, = nodes.common_ksampler(
        ...
        positive=positive, negative=negative, latent=latent_dict,
        denoise=float(denoise), disable_noise=disable_noise,
...
def _encode_seed_image_vision(clip_vision, seed_image, cropping="center"):
    """Encodes the seed image using CLIP Vision, handling potential errors."""
    ...
    if image_tensor.dim() == 4 and image_tensor.shape[1] in (1, 3, 4):
        image_tensor = image_tensor.permute(0, 2, 3, 1).contiguous()
    if image_tensor.dim() != 4 or image_tensor.shape[-1] not in (1, 3, 4):
        _log(f"Warning: Invalid image shape for CLIP Vision: {tuple(image_tensor.shape)}")
        return None
    vision_output = clip_vision.encode_image(image_tensor, crop=(cropping == "center"))
    return vision_output

def _tokenize_and_encode(clip, positive_prompt, negative_prompt):
    """Encodes positive and negative text prompts using the provided CLIP model."""
...
def _prepare_latent_window(positive_cond, negative_cond, vae, width, height, frame_count, batch_size, seed_image=None, vision_output=None):
    """Creates the latent tensor window and conditions it with seed image/CLIP Vision."""
    device = comfy.model_management.intermediate_device()
    latent_height = height // 8
    latent_width = width // 8
    time_dim = ((frame_count - 1) // 4) + 1
...
class WanVideoLooper:
    ...
    def loop_video(
        ...
        # --- Check if model_clip_sequence is valid, ignore if bypassed ---
        ...
        for i, prompt_text in enumerate(positive_prompts):
            ...
            # Use sequence only if it's a valid list (handles bypass)
            ...
            if not enable_dry_run:
                ...
                if enable_motion_cfg and switch_step_index > 1:
                    ...
                    initial_latent = _run_sampler_step(
                        loop_model_high, current_seed, total_steps, cfg_motion_step, sampler_name, scheduler,
                        loop_positive_cond, loop_negative_cond, latent_on_gpu, denoise,
                        disable_noise=False, start_step=0, last_step=1, force_full_denoise=False
                    )
                    high_noise_latent = _run_sampler_step(
                        loop_model_high, current_seed, total_steps, cfg_high_noise, sampler_name, scheduler,
                        loop_positive_cond, loop_negative_cond, initial_latent, denoise,
                        disable_noise=True, start_step=1, last_step=switch_step_index, force_full_denoise=False
                    )
                else:
                    high_noise_latent = _run_sampler_step(
                        loop_model_high, current_seed, total_steps, cfg_high_noise, sampler_name, scheduler,
                        loop_positive_cond, loop_negative_cond, latent_on_gpu, denoise,
                        disable_noise=False, start_step=0, last_step=switch_step_index, force_full_denoise=False
                    )

                _log(f"Segment {i+1}: Running LOW noise sampler (Steps {switch_step_index} to {total_steps}).")
                final_latent = _run_sampler_step(
                    loop_model_low, current_seed, total_steps, cfg_low_noise, sampler_name, scheduler,
                    loop_positive_cond, loop_negative_cond, high_noise_latent, denoise,
                    disable_noise=True, start_step=switch_step_index, last_step=total_steps, force_full_denoise=True
                )
                _log(f"Segment {i+1}: Sampling finished.")
            else:
                _log(f"Segment {i+1}: DRY RUN - Skipping HIGH noise sampler.")
                _log(f"Segment {i+1}: DRY RUN - Skipping LOW noise sampler.")
                final_latent = latent_on_gpu
                high_noise_latent = latent_on_gpu
        ...
        for batch in all_frames_collected:
             if batch.shape[-1] == num_channels:
                 ...
                 elif batch.shape[-1] == 3 and num_channels == 4:
                      alpha = torch.ones_like(batch[:, :, :, :1]); batch = torch.cat([batch, alpha], dim=-1)
